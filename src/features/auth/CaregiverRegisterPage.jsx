...
function Note({ icon: Icon, children, tone = 'neutral' }) {
  ...
  return (
    <p className={cn('flex items-start gap-2.5 rounded-xl border px-3.5 py-3 text-xs leading-relaxed', tones[tone])}>
            ...
            <form onSubmit={handleSubmit} className="mt-7 space-y-5">
              ...
              <div>
                  ...
                  <AnimatePresence initial={false}>
                    {documents.map((doc, i) => (
                      <motion.li
                        ...
                        className="rounded-2xl border border-line bg-slate-900/[0.015] p-3.5"
                      >
                        <div className="flex items-start gap-3">
                          <span className="mt-2.5 font-mono text-[10px] tracking-[0.14em] text-slate-400">
                            {String(i + 1).padStart(2, '0')}
                          </span>

                          <div className="grid min-w-0 flex-1 gap-3">
                            <Input
                              label={t('caregiverRegister.documentNameLabel')}
                          ...
                          <motion.button
                            ...
                            className="mt-1.5 shrink-0 cursor-pointer rounded-full border border-line bg-surface p-1.5 text-slate-400 hover:border-rose-600/40 hover:text-rose-600"
                ...
                <button
                  type="button"
                  onClick={addDocument}
                  className="mt-3 inline-flex cursor-pointer items-center gap-2 rounded-full border border-dashed border-line-strong bg-slate-900/[0.02] px-4 py-2.5 text-[13px] font-semibold text-slate-600 transition-colors duration-300 hover:border-brand-600/50 hover:bg-brand-600/[0.05] hover:text-brand-700"
                >
                  <Plus size={15} />
              ...
              <AnimatePresence>
                {error && (
                  <motion.p
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0 }}
                    className="rounded-xl border border-rose-600/30 bg-rose-600/10 px-3.5 py-2.5 text-xs font-medium text-rose-500"
                  >
                    {error}
                  </motion.p>
                )}
